	...
	$.ajax({	
			...
			success: function(result){
				...
				if(result.available && result.user_signed_in){
	        		...
	        		$("#slots td").unbind( "click" ).click(function(){ setTimeFromSlot($(this).html())});
					...
					var html="";
	        		for(i=0;i<result.time_slots.length;i++){
	        			html = html+"<tr data-value = '"+result.time_slots[i].id+"'>";
					...
					for(x=0;x<result.time_slots.length;x++){
						html=html+"<tr data-value='"+result.time_slots[x].id+"'><th>"+result.time_slots[x].title+"</th></tr>"
		        		for(i=0;i<result.time_slots[x].time_slots.length;i++){
		        			html = html+"<tr data-value='"+result.time_slots[x].time_slots[i].id+"'>";
