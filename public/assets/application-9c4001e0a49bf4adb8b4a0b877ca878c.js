...
function populateBranches(val){
	...
	if(val==0){
		...
		$.ajax({url: "/dashboard/restaurants/"+val+"/branches/list", success: function(result){
				var html = "";
				$("#reservation_branch_id").val("");
	...
	$.ajax({	
			...
			success: function(result){
				...
				if(result.available && result.user_signed_in){
					...
					html1 = html1+"<tr>";
	        		for(i=0;i<result.time_slots.length;i++){
	        			html1 = html1+"<th>"+result.time_slots[i].title+"</th>";
	        			html2 = html2+"<tr>";
	        			for(j=0;j<result.time_slots[i].time_slots.length;j++){
		        			if(result.time_slots[i].time_slots[j].available){
		        				html2= html2 + "<td>"+result.time_slots[i].time_slots[j].time_slot+"</td>";
		        			}
		        		}
		        		html2 = html2 + "</tr>";
	        		}
	        		html1 = html1 + "</tr>";
	        		$("table#slots tbody").html(html1+html2);
	        		$('#time-slots').modal('show');
				}
					...
					for(x=0;x<result.time_slots.length;x++){
						html=html+"<tr><th>"+result.time_slots[x].title+"</th></tr>"
