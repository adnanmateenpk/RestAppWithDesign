	...
	$.ajax({	
			...
			success: function(result){
				if(result.available){
	        		...
	        		for(i=0;i<result.time_slots.length;i++){
	        			html = html + "<tr>"
		        		for(i=0;i<result.time_slots[i].time_slots.length;i++){
		        			if(result.time_slots[i].time_slots[i].available){
		        				html= html + "<td>"+result.time_slots[i].time_slots[i].time_slot+"</td>";
		        			}
		        		}
		        		html = html + "</tr>"
	        		}
