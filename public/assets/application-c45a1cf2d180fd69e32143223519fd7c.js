	...
	if(val==0){
		...
		$.ajax({url: "/dashboard/restaurants/"+val[1]+"/branches/list", success: function(result){
	            ...
	            for (i = 0 ;i<result.length;i++){
	            	...
	            	$("#reservation_time_zone").val(result[0].time_zone);
...
function assignBranchValue(val){
	...
	$("#reservation_time_zone").val(val[1]);
}
function checkAvailability(id){
	...
	$.ajax({	
			...
			type: "POST",
			data:{"customer":user,"branch" : val,"time":time,"date":date,"people" : people,"id":id,"time_zone" : $("#reservation_time_zone").val()},
			error: function(){
