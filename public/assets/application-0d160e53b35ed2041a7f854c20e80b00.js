		...
		$.ajax({url: "/dashboard/restaurants/"+val[1]+"/branches/list", success: function(result){
	            ...
	            if(result.length>1){
		            for (i = 0 ;i<result.length;i++){
		            	html = html+"<option style='border-top:1px solid;' onclick='assignBranchValue(\""+result[i].id +"|"+result[i].time_zone+"|"+result[i].seating_capacity+"\");' value=>"+result[i].title+"</option>";
		            }
		        }
