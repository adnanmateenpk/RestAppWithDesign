...
function checkTables(user){
	val = $("#reservation_branch_id").val();
	people = $("#reservation_poeple").val();
	date = $("#date").val();
	time = $("#time").val();

	if(people && parseInt(people)!=0 && val && parseInt(val)!=0){
		$.ajax({	
			url: "/availability",
			type: "POST",
			data:{"customer":user,"branch" : $("#reservation_branch_id").val(),"time":time,"date":date },
		...
		alert("Please add a valid no of people");
	}
