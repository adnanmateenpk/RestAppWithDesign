...
function populateBranches(val){
	if(val==0){
		$("#reservation_branch_id").html("");
		$("#reservation_table_id").html("");
	}
	else {
		val = val.split("|");
		$.ajax({url: "/dashboard/restaurants/"+val[1]+"/branches/list", success: function(result){
			if(result.length>0){
	            ...
	            $("#reservation_branch_id").html(html);
        	}
        }});
	}
	
}
function populateTables(val){
	if(val==0){
		$("#reservation_table_id").html("");
	}
	else {
		val = val.split("|");
		$.ajax({url: "/dashboard/restaurants/"+val[2]+"/branches/"+val[1]+"/tables/list", success: function(result){
			if(result.length>0){
	            html="<option value='0'>Select One</option>";
	            for (i = 0 ;i<result.length;i++){
	            	html = html+"<option value='"+result[i].id+"'>"+result[i].title+"</option>";
	            }
	            $("#reservation_table_id").html(html);
        	}
        }});
	}
